...
def test_bench_flush_threshold_sweep(benchmark, medium_payload, flush_threshold):
    ...
    def run():
        return asyncio.run(_run_streaming(
            medium_payload, chunk_size=4096, flush_threshold=flush_threshold,
        ))
    result = benchmark(run)
    assert result > 0
...
def test_bench_eager_vs_batched(benchmark, medium_payload):
    ...
    def run_eager():
        return asyncio.run(_run_streaming(
            medium_payload, chunk_size=4096,
...
def test_bench_large_document(benchmark, large_payload):
    ...
    def run():
        return asyncio.run(_run_streaming(
            large_payload, chunk_size=16384, flush_threshold=64 * 1024,
        ))
    benchmark(run)